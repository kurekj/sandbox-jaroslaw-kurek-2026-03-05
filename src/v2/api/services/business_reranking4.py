...
def business_score_breakdown(
    ...
    shares_signed_abs_pct = {f"{k}_share_signed_abs_pct": (v / abs_total) * 100.0 for k, v in contribs.items()}

    # Share vs final score (can be unstable when score_final ≈ 0)
    if abs(score_final) > 1e-12:
        shares_of_score_final_pct = {
            f"{k}_share_of_score_final_pct": (v / score_final) * 100.0 for k, v in contribs.items()
        }
    else:
        shares_of_score_final_pct = {f"{k}_share_of_score_final_pct": None for k in contribs.keys()}
    ...
    breakdown: Dict[str, Any] = {
        ...
        **contribs,
        ...
        # contrib checks / totals
        "contrib_abs_total": abs_total,
        "contrib_sum_check": float(sum(contribs.values())),
        "contrib_sum_error": float(sum(contribs.values()) - score_final),

        # abs shares (%)
        **shares_abs_pct,
        **shares_signed_abs_pct,
        **shares_of_score_final_pct,
