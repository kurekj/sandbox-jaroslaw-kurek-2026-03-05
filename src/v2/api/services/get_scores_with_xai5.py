...
def coerce_uuid(value: Any) -> uuid.UUID:
    """Convert different types of ``user_id`` values to ``uuid.UUID``.

    The recommendation database stores user identifiers as UUIDs.  This
    helper accepts strings, integers or UUID objects and attempts to
    return a valid ``uuid.UUID`` instance.  Integers are deterministically
    converted into UUIDs so that even numeric identifiers can be used
    during development.  If conversion fails, a namespace‑based UUID is
    generated from the string representation of the input.
    ...
    -------
    uuid.UUID
        A UUID representing the user identifier.
    """
    # Already a UUID
    if isinstance(value, uuid.UUID):
        return value
    # Try to parse from string form
    try:
        return uuid.UUID(str(value))
    except Exception:
        pass
    # Try deterministic conversion from integer
    try:
        return uuid.UUID(int=int(value))
    except Exception:
        pass
    # Fallback: generate a namespace UUID based on the string
