...
if __name__ == "__main__":
    ...
    # Configure logging: log to both console (default) and a file
    log_file_name = "xai_report.log"
    try:
        # Add a log file sink
    ...
    for entry in explanations:
        ...
        # Print a succinct view of the top contributing features
        # Sort features by absolute SHAP value descending and take top 5
        top_features = sorted(
            entry["feature_contributions"].items(),
        ...
        print("Top contributing features (feature: SHAP value):")
        for feat, val in top_features:
            print(f"  {feat}: {val:.4f}")
        print("---")

    # Save the complete explanations list to a JSON file
    json_file_name = "xai_explanations.json"
    with open(json_file_name, "w", encoding="utf-8") as f:
        json.dump(explanations, f, ensure_ascii=False, indent=2)
    logger.info(f"Saved XAI report for {len(explanations)} entries to {json_file_name}")
    print(f"\nFull JSON report saved to {json_file_name} (see also log file {log_file_name})")