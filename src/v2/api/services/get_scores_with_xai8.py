...
async def get_scores_with_explanations(
    ...
    # feature names to the corresponding SHAP value.  For each row, a
    # list of feature objects is built containing the raw feature name,
    # mapped name, raw SHAP contribution, and the percentage contribution
    # relative to the sum of absolute contributions.  Both the unsorted
    # list (feature_contributions) and a sorted version (sorted_contributions)
    # are returned in each explanation entry.  Casting floats to native
    # Python types ensures JSON serialization works correctly.
    explanations: List[Dict[str, Any]] = []
    for idx, row in merged_df.iterrows():
        shap_row = shap_values[idx]
        # Compute total absolute contribution for percentage calculations
        total_abs = float(np.sum(np.abs(shap_row))) if shap_row is not None else 0.0
        # Build a list of feature contribution objects with original order (1-based)
        feature_list: List[Dict[str, Any]] = []
        for col_idx, feature in enumerate(EXPECTED_FEATURE_COLUMNS):
            contrib_value = float(shap_row[col_idx])
            # Compute percentage contribution; avoid division by zero
            pct = (abs(contrib_value) / total_abs) * 100 if total_abs != 0 else 0.0
            feature_list.append(
                {
                    "order": col_idx + 1,
                    "feature": feature,
        ...
        # Sort by absolute contribution and assign new ordinal numbers in sorted order
        sorted_contrib: List[Dict[str, Any]] = []
        for sort_idx, item in enumerate(
            sorted(feature_list, key=lambda x: abs(x["contribution"]), reverse=True), start=1
        ):
            # Create a shallow copy to avoid mutating the original list
            new_item = item.copy()
            new_item["order"] = sort_idx
            sorted_contrib.append(new_item)
        explanations.append(
            {
