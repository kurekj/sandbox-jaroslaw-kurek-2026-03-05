...
# Ile prób lokalnego doszlifowania wokół best (gauss multiplikatywny)
N_LOCAL_TRIALS = 150
LOCAL_REL_STD = 0.15  # im mniejsze, tym drobniejszy search wokół najlepszego

# Opcjonalne przyspieszenie: oceniaj na próbce requestów w fazie search
# (Top-N później jest liczony na pełnym train/valid)
TRAIN_FRACTION = 0.8
TRAIN_SAMPLE_REQUESTS: Optional[int] = 5000  # None => full train w każdym trialu
TOP_N_REEVAL = 40  # ile najlepszych triali przeliczyć na full train/valid

# Zakresy parametrów (sensowne defaulty: 0..3)
...
def main() -> None:
    ...
    for t in tqdm(range(1, N_RANDOM_TRIALS + 1), desc="Random search", unit="trial"):
        p_dict, p_h = sample_params_uniform(rng, best_params_dict)
    ...
    for i in tqdm(range(N_LOCAL_TRIALS), desc="Local refine", unit="trial"):
        p_dict, p_h = perturb_params_local(rng, best_params_dict, best_p_h, rel_std=LOCAL_REL_STD)
        params = RerankParams(**p_dict)
        ...
        row = {"trial_id": start_local_id + i, "stage": "local", **p_dict, "p_h": p_h, "train_profit_sample": profit_sample}
        trials.append(row)
