...
RECS_PATH = Path("recommendations_output_2026-02-20_23-59-59.parquet")
...
def build_tradeoff_request_level(
    *,
    ...
    df["expected_revenue"] = pd.to_numeric(_series_or_nan(df, "expected_revenue"), errors="coerce").fillna(0.0).astype(float)
    df["unit_value_pos"] = pd.to_numeric(_series_or_nan(df, "unit_value_pos"), errors="coerce").fillna(0.0).astype(float)
    df["position_weight"] = pd.to_numeric(_series_or_nan(df, "position_weight"), errors="coerce").fillna(0.0).astype(float)
    df["is_billable"] = pd.to_numeric(_series_or_nan(df, "is_billable"), errors="coerce").fillna(0).astype(int)

    df["q_prob"] = pd.to_numeric(_series_or_nan(df, "q_prob"), errors="coerce").fillna(0.0).clip(0.0, 1.0).astype(float)
    df["q_api"] = pd.to_numeric(_series_or_nan(df, "q_api"), errors="coerce").fillna(0.0).clip(-1.0, 1.0).astype(float)
    df["q_final"] = pd.to_numeric(_series_or_nan(df, "q_final"), errors="coerce").fillna(0.0).clip(-1.0, 1.0).astype(float)
    ...
    denom = pd.to_numeric(req["pos_w_sum"], errors="coerce").replace({0.0: np.nan})
    req["q_prob_wmean"] = (req["q_prob_w_sum"] / denom).fillna(0.0).astype(float)
    req["q_api_wmean"] = (req["q_api_w_sum"] / denom).fillna(0.0).astype(float)
    req["q_final_wmean"] = (req["q_final_w_sum"] / denom).fillna(0.0).astype(float)
    req["rank_score_wmean"] = (req["rank_score_w_sum"] / denom).astype(float)
