...
# --- NEW: daily_snapshot.offer_performance_metrics snapshot inputs ---
# Join key between recs and snapshot: offer_id
...
def read_offer_performance_metrics(path: Path) -> pd.DataFrame:
    ...
    Expected columns:
    ['id','day','offer_id','group_id','monthly_budget','estimated_monthly_budget','group_budget',
     'lead_limit','leads','leads_estimation','estimated_perc_realization','lead_price',
     'estimated_lead_price','settlement_type']
    """
    if not path.exists():
...
def enrich_with_offer_performance_metrics(
    ...
    Join strategy:
      - Key: offer_id
      - Time alignment:
        rec_day = floor(create_date to date) + OFFERS_OPM_DAY_SHIFT_DAYS
        merge_asof per offer_id to take the last snapshot day <= rec_day (by default).

    Output columns added:
      - rec_day
      - snapshot fields (day, group_id, budgets, leads, limits, etc.)
    ...
    # Normalize offer_id to numeric (so merge_asof by='offer_id' is consistent)
    out["offer_id"] = pd.to_numeric(out["offer_id"], errors="raise").astype("Int64")
    ...
    # merge_asof requires both frames sorted by the merge key (on)
    ...
    # --- Map offer_performance_metrics -> Candidate features ---

    # contract_type from settlement_type (0=CPL, 1=flat/subscription)
    ...
    # inv_id: prefer group_id from snapshot; fallback to offer_id
    ...
    # m: budget per offer:
    #  - CPL: monthly_budget (non-null)
    #  - flat/subscription: estimated_monthly_budget (non-null)
    ...
    # v: plan/limit leadów (lead_limit); brak -> 0
    merged["v"] = (
    ...
    # cap_ratio:
    # prefer estimated_perc_realization; fallback to leads/lead_limit if needed
    ...
    fallback_any = (
        (merged["id"].isna() if "id" in merged.columns else True)
        | (merged["contract_type"].isna())
        | (merged["inv_id"].isna())
    )
    fallback_candidates = int(pd.Series(fallback_any).sum())
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Important:
      - If df already has columns: m, v, contract_type, cap_ratio, inv_id
        (e.g. after offer_performance_metrics enrichment), we DO NOT overwrite them.
      - Otherwise, we fall back to DEFAULT_* values.
    """
...
def main() -> None:
    ...
    # Enrich with offer_performance_metrics snapshot if available.
    # If not available, the script keeps working on DEFAULT_* values.
...
# CHANGES vs ver0.03
# =========================
# - Replaced daily_snapshot.offers_billinggroup integration with daily_snapshot.offer_performance_metrics (XLSX dump).
# - Join key is offer_id (as per your note).
# - Implemented read_offer_performance_metrics() with required-column validation and dtype parsing.
# - Implemented enrich_with_offer_performance_metrics():
#     * aligns create_date -> rec_day and uses merge_asof by offer_id to pick last snapshot day <= rec_day
#     * contract_type from settlement_type (0=cpl, 1=flat)
#     * cap_ratio from estimated_perc_realization (fallback: leads/lead_limit), clipped to [0,1]
#     * inv_id prefers group_id, fallback to offer_id
#     * m uses monthly_budget (CPL) or estimated_monthly_budget (flat)
#     * v uses lead_limit (int)
