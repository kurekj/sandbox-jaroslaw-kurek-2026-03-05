...
def compute_pair_metrics(
    ...
    overlap_col = f"top{k}_overlap_cnt"
    if overlap_col not in summary_df.columns:
        raise ValueError(f"Missing column: {overlap_col}")

    overlap = pd.to_numeric(summary_df[overlap_col], errors="coerce").fillna(0)

    # top1 match
    overlap_at_1 = (
        pd.to_numeric(summary_df[f"{left_prefix}_pid_1"], errors="coerce")
        == pd.to_numeric(summary_df[f"{right_prefix}_pid_1"], errors="coerce")
    ).fillna(False)
    ...
    signed_deltas = []
    for i in range(1, k + 1):
        lsc = pd.to_numeric(summary_df[f"{left_prefix}_score_{i}"], errors="coerce")
        rsc = pd.to_numeric(summary_df[f"{right_prefix}_score_{i}"], errors="coerce")
        d = lsc - rsc
        abs_deltas.append(d.abs())
        signed_deltas.append(d)

    abs_delta = pd.concat(abs_deltas, axis=1)
    signed_delta = pd.concat(signed_deltas, axis=1)

    mean_abs_delta_per_row = abs_delta.mean(axis=1, skipna=True)
    mean_signed_delta_per_row = signed_delta.mean(axis=1, skipna=True)

    return {
        "requests": int(n),
        "avg_overlap_ratio": float((overlap / k).mean()) if n else 0.0,
        "pct_full_topk_match": float(full_match.mean()) if n else 0.0,
        "pct_top1_match": float(overlap_at_1.mean()) if n else 0.0,
        "avg_abs_score_delta": float(mean_abs_delta_per_row.mean(skipna=True)) if n else 0.0,
        "avg_signed_score_delta": float(mean_signed_delta_per_row.mean(skipna=True)) if n else 0.0,
    }
...
def build_metrics_sheet(
    ...
    m1 = compute_pair_metrics(sum_base_pg, k=k, left_prefix="base", right_prefix="pg")
    rows.append({"comparison": "baseline vs propertygroup", **m1})

    m2 = compute_pair_metrics(sum_base_kurekj, k=k, left_prefix="base", right_prefix="kurekj")
    rows.append({"comparison": "baseline vs kurekj", **m2})

    m3 = compute_pair_metrics(sum_pg_kurekj, k=k, left_prefix="pg", right_prefix="kurekj")
    rows.append({"comparison": "propertygroup vs kurekj", **m3})

    df = pd.DataFrame(rows)

    df["avg_overlap_ratio_pct"] = (df["avg_overlap_ratio"] * 100.0).round(2)
    df["pct_full_topk_match_pct"] = (df["pct_full_topk_match"] * 100.0).round(2)
    df["pct_top1_match_pct"] = (df["pct_top1_match"] * 100.0).round(2)
...
def main() -> None:
    ...
    metrics_df = build_metrics_sheet(
        k=TOPK,
        sum_base_pg=sum_base_pg,
        sum_base_kurekj=sum_base_kurekj,
        sum_pg_kurekj=sum_pg_kurekj,
    )
    ...
    with pd.ExcelWriter(out_path, engine=engine) as xw:
        ...
        metrics_df.to_excel(excel_writer=xw, sheet_name="metrics", index=False)
