...
def business_topk(
    df: pd.DataFrame, params: RerankParams, k: int, p_h: float
) -> pd.DataFrame:
...
def summary_wide(
    baseline: pd.DataFrame, business: pd.DataFrame, k: int
) -> pd.DataFrame:
    meta = (
    ...
    base_w = baseline.pivot(
        index="request_id", columns="baseline_rank_final", values="property_id"
    )
    base_w.columns = [f"baseline_property_id_{c}" for c in base_w.columns]

    biz_w = business.pivot(
        index="request_id", columns="business_rank", values="property_id"
    )
...
def main() -> None:
    ...
    # ===== OUTPUT FOLDER WITH DATESTAMP =====
    script_dir = Path(__file__).resolve().parent
    datestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = script_dir / f"Output_{datestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)

    out_xlsx_path = out_dir / OUT_XLSX_FILENAME
    # =======================================
    ...
    print(
        f"baseline rows={len(base_top)}, "
        f"business rows={len(biz_top)}, "
        f"summary rows={len(summ)}"
    )
