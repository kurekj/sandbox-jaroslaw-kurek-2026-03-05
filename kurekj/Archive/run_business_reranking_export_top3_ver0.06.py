...
def summary_wide_pair(
    ...
    l.columns = [f"{left_prefix}_property_id_{c}" for c in l.columns]

    r = right.pivot(index="request_id", columns=right_rank, values="property_id")
    r.columns = [f"{right_prefix}_property_id_{c}" for c in r.columns]

    out = meta.join(l).join(r).reset_index()

    # FIX: pandas DataFrame nie ma setdefault
    for i in range(1, k + 1):
        left_col = f"{left_prefix}_property_id_{i}"
        right_col = f"{right_prefix}_property_id_{i}"
    ...
    def overlap(row):
        a = {row[f"{left_prefix}_property_id_{i}"] for i in range(1, k + 1)}
        b = {row[f"{right_prefix}_property_id_{i}"] for i in range(1, k + 1)}
...
def main() -> None:
    ...
    propertygroup_business_top = business_topk_by(
        recs,
        params,
        TOPK,
        P_H,
        q_col="q_final",
        rank_col="propertygroup_business_rank",
        score_col="propertygroup_business_score",
        desc="Propertygroup reranking (final_score)",
    )

    kurekj_business_top = business_topk_by(
        recs,
        params,
        TOPK,
        P_H,
        q_col="q_api",
        rank_col="kurekj_business_rank",
        score_col="kurekj_business_score",
        desc="Kurekj reranking (api_model_score)",
    )

    summary_baseline_vs_propertygroup = summary_wide_pair(
        baseline_top,
        propertygroup_business_top,
        TOPK,
        "baseline_rank",
        "propertygroup_business_rank",
        "baseline",
        "propertygroup",
    )

    summary_baseline_vs_kurekj = summary_wide_pair(
        baseline_top,
        kurekj_business_top,
        TOPK,
        "baseline_rank",
        "kurekj_business_rank",
        "baseline",
        "kurekj",
    )

    summary_propertygroup_vs_kurekj = summary_wide_pair(
        propertygroup_business_top,
        kurekj_business_top,
        TOPK,
        "propertygroup_business_rank",
    ...
    with pd.ExcelWriter(out_dir / OUT_XLSX_FILENAME, engine="openpyxl") as xw:
        baseline_top.to_excel(xw, "baseline_topk_long", index=False)
        propertygroup_business_top.to_excel(xw, "propertygroup_business_topk_long", index=False)
        kurekj_business_top.to_excel(xw, "kurekj_business_topk_long", index=False)

        summary_baseline_vs_propertygroup.to_excel(
            xw, "summary_baseline_vs_propertygroup", index=False
        )
        summary_baseline_vs_kurekj.to_excel(
            xw, "summary_baseline_vs_kurekj", index=False
        )
        summary_propertygroup_vs_kurekj.to_excel(
            xw, "summary_propertygroup_vs_kurekj", index=False
        )
