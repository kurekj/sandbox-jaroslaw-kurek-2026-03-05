...
OUTPUT_PATH = "recommendations_output.parquet"
...
WITH ranked AS (
    ...
    WHERE    is_production
    AND      create_date >= '2025-12-08 12:02:00'::timestamp - interval '6 days 18 hours'
    AND      create_date <= '2025-12-08 12:02:00'::timestamp
),
per_request AS (
...
async def run_query_and_save():
    ...
    rows = await execute_query(QUERY)  # list[dict]

    print(f"Fetched {len(rows)} rows. Converting to DataFrame...")
    df = pd.DataFrame(rows)

    # 🔧 KONWERSJA PROBLEMATYCZNYCH KOLUMN

    # request_id jest typu uuid.UUID → zamieniamy na tekst
    if "request_id" in df.columns:
        df["request_id"] = df["request_id"].astype(str)

    # uuid z zapytania jest już tekstem (->> 'uuid'), ale gdyby jednak był UUID:
